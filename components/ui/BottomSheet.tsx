  ...
  const handleDragEnd = useCallback(
    (_: unknown, info: { offset: { y: number }; velocity: { y: number } }) => {
      ...
      if (shouldClose) {
        ...
        controls.start({ y: 0, transition: spring.snappy });
      }
    },
    [controls, onClose]
  );
        ...
        <>
          ...
          <motion.div
            ...
            className={cn(
              ...
              "flex flex-col",
              "rounded-t-[var(--c-sheet-radius)] overflow-hidden",
              "bg-[var(--c-sheet-bg)] backdrop-blur-2xl",
              "border-t border-l border-r border-[var(--c-sheet-border)]",
              "shadow-[0_-8px_40px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,255,255,0.10)]",
              "pb-safe",
              maxH,
              className
            )}
          >
            ...
            >
              <motion.div
                className="h-1 w-10 rounded-full bg-[var(--s-border-emphasis)]"
                whileTap={{ scaleX: 1.3 }}
              />
            </div>
            ...
            {title && (
              <div className="flex items-center justify-between px-5 pb-3">
                <h2 id={titleId} className="text-base font-black text-[var(--s-text-primary)]">{title}</h2>
                <motion.button
                  whileTap={{ scale: 0.9 }}
                  onClick={onClose}
                  aria-label="Chiudi"
                  className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--s-bg-glass)] text-[var(--s-text-tertiary)] hover:text-[var(--s-text-primary)]"
                >
                  <X size={16} />
                </motion.button>
              </div>
