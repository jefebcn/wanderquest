      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40">Ristoranti, monumenti, nightlife e molto altro</p>
          </div>
        </div>
                      ...
                      <div key={i} className="flex items-center gap-3 rounded-xl bg-white/[0.04] border border-white/8 px-4 py-3">
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="flex items-center gap-1 justify-end">
                            <Star size={10} className="text-[var(--s-primary)]" fill="currentColor" />
                            <span className="text-[12px] font-black text-[var(--s-primary)]">{r.rating}</span>
                          </div>
                          <span className="text-xs text-white/35">{r.priceRange}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={i} className="flex items-start gap-3 rounded-xl bg-white/[0.03] border border-white/8 px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {m.mustSee && (
                              <span className="rounded-full bg-[var(--s-primary)]/15 border border-[var(--s-primary)]/25 px-1.5 py-0.5 text-xs font-black text-[var(--s-primary)]">
                                DA VEDERE
                              </span>
                            )}
                          </div>
                          <p className="text-xs text-white/45 mt-0.5 leading-snug">{m.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={i} className="flex items-start gap-3 rounded-xl bg-purple-500/[0.05] border border-purple-500/15 px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="rounded-full bg-purple-500/20 border border-purple-500/25 px-1.5 py-0.5 text-xs font-bold text-purple-300">
                              {n.type}
                            </span>
                          </div>
                          <p className="text-xs text-white/45 mt-0.5 leading-snug">{n.description}</p>
                          <p className="text-xs text-purple-300/50 mt-1 italic">{n.vibe}</p>
                        </div>
                      </div>
                    ))}
                  </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2.5 right-2.5">
        {city.active ? (
          <span className="flex items-center gap-1 rounded-full bg-green-500/20 border border-green-500/40 px-2 py-0.5 text-xs font-black text-green-400 backdrop-blur-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-green-400 animate-pulse inline-block" />
            LIVE
          </span>
        ) : (
          <span className="rounded-full bg-white/10 border border-white/15 px-2 py-0.5 text-xs font-bold text-white/50 backdrop-blur-sm">
            Presto
          </span>
        )}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3">
        <p className="text-xs font-bold uppercase tracking-widest text-white/40 mb-0.5">{city.country}</p>
        <p className="font-black text-white text-sm leading-tight">{city.name}</p>
        <p className="text-xs text-white/55 mt-0.5">{city.landmarks} monumenti</p>
      </div>
    </motion.div>
  );
}
        ...
        <div className="pb-10">
          ...
          <div className="relative h-52 w-full overflow-hidden rounded-t-none">
            ...
            <div className="absolute top-4 right-4">
              {city.active ? (
                <span className="flex items-center gap-1.5 rounded-full bg-green-500/80 border border-green-400/50 px-3 py-1 text-xs font-black text-white backdrop-blur-sm">
                  <span className="h-1.5 w-1.5 rounded-full bg-white animate-pulse" />
                  LIVE ORA
                </span>
              ) : (
                <span className="rounded-full bg-black/50 border border-white/25 px-3 py-1 text-xs font-bold text-white/80 backdrop-blur-sm">
          ...
          <div className="px-5 pt-5 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-xl bg-white/[0.05] border border-white/10 p-3 text-center">
                <p className="text-xl font-black text-[var(--s-primary)]">{city.landmarks}</p>
                <p className="text-xs text-white/40 mt-0.5">Monumenti</p>
              </div>
              <div className="rounded-xl bg-white/[0.05] border border-white/10 p-3 text-center">
                <p className="text-xl font-black text-[var(--s-primary)]">
                  {details.topLandmarks.reduce((a, l) => a + l.pts, 0)}+
                </p>
                <p className="text-xs text-white/40 mt-0.5">Punti top 3</p>
              </div>
              <div className="rounded-xl bg-white/[0.05] border border-white/10 p-3 text-center">
                <p className="text-xl font-black text-white/60">{city.active ? "🟢" : "🔜"}</p>
                <p className="text-xs text-white/40 mt-0.5">{city.active ? "Disponibile" : "In arrivo"}</p>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {details.highlights.map((h) => (
                  <div key={h} className="flex items-center gap-2">
                    <CheckCircle2 size={13} className="text-[var(--s-primary)] flex-shrink-0" />
              ...
              <div className="grid grid-cols-3 gap-2">
                {details.topLandmarks.map((lm) => (
                  <div key={lm.name} className="rounded-xl bg-white/[0.05] border border-white/8 p-3 text-center">
                    ...
                    <p className="text-xs font-bold text-white/70 leading-tight line-clamp-2">{lm.name}</p>
                    <p className="text-xs font-black text-[var(--s-primary)] mt-1">+{lm.pts}pt</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {city.active ? (
              user ? (
                <Link
                  href="/scan"
                  onClick={onClose}
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--s-primary)] py-4 text-sm font-black text-slate-900 shadow-[0_4px_24px_rgba(255,215,0,0.32)]"
                >
                  <Navigation2 size={16} />
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-center">
                ...
                <p className="text-2xl font-black text-[var(--s-primary)]">{details.openDate ?? "Prossimamente"}</p>
...
function AnimatedPrize({ targetCents }: { targetCents: number }) {
  ...
  return (
    <span className="text-3xl font-black text-[var(--s-primary)] tabular-nums">
      €{euros}
    </span>
  );
}
...
export default function HomePage() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--s-bg-base)]">
  ...
  return (
    <>
      <div className="min-h-screen bg-[var(--s-bg-base)] text-white overflow-x-hidden pb-28">
        ...
        <section className="relative min-h-[92svh] flex flex-col justify-end overflow-hidden">
          <div className="absolute inset-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[var(--s-bg-base)] via-[var(--s-bg-base)]/65 to-[var(--s-bg-base)]/5" />
            <div className="absolute inset-0 bg-gradient-to-r from-[var(--s-bg-base)]/55 via-transparent to-transparent" />
            {/* Subtle gold glow at horizon */}
            <div className="absolute bottom-0 left-0 right-0 h-56 bg-gradient-to-t from-[var(--s-primary)]/7 to-transparent" />
          </div>
          ...
          <div className="relative z-10 px-5 pb-10">
            ...
            {contest && (
              <motion.div
                ...
                className="inline-flex items-center gap-2 rounded-full bg-[var(--s-primary)]/12 border border-[var(--s-primary)]/30 px-4 py-1.5 mb-5"
              >
                <Sparkles size={12} className="text-[var(--s-primary)]" />
            ...
            <motion.h1
              ...
              className="font-display text-[2.9rem] leading-[1.04] font-black mb-4"
            >
              Ogni Angolo<br />Nasconde
              <br />
              <span style={{
                background: "linear-gradient(135deg,var(--s-primary) 0%,#FFA500 100%)",
            ...
            {user ? (
              ...
              >
                <Link
                  href="/scan"
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--s-primary)] py-4 text-[15px] font-black text-slate-900 shadow-[0_6px_32px_rgba(255,215,0,0.40)] hover:bg-yellow-300 transition-colors min-h-[52px]"
                >
                  <Trophy size={18} />
                  Vai al Contest
          ...
          <div className="grid grid-cols-2 gap-3">
            {steps.map(({ step, icon: Icon, title, desc, gradient, border, iconBg, iconColor, glow }, i) => (
              ...
              >
                ...
                <p className="text-xs text-white/48 leading-snug">{desc}</p>
              </motion.div>
            ))}
          </div>
        </section>
                    ...
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex gap-0.5">
                        {Array.from({ length: stars }).map((_, s) => (
                          <Star key={s} size={10} className="text-[var(--s-primary)]" fill="currentColor" />
                        ))}
                      </div>
                    </div>
        ...
        <section className="px-4 mb-10">
          ...
          >
            ...
            {user ? (
              <Link
                href="/scan"
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--s-primary)] py-4 text-sm font-black text-slate-900 shadow-[0_4px_24px_rgba(255,215,0,0.32)] hover:bg-yellow-300 transition-colors min-h-[48px]"
              >
                <Trophy size={16} />
                Vai al Contest
              </Link>
